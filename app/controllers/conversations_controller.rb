class ConversationsController < ApplicationController
  ...
  def create
    ...
    if @conversation.save
      ...
      # AI処理を別スレッドで実行
      process_ai_response(original_text)
    else
      # バリデーションエラー時の処理
      render turbo_stream: turbo_stream.replace("conversation-form", partial: "form", locals: { conversation: @conversation })
    end
  end

  private

  def process_ai_response(original_text)
    # AI処理をバックグラウンドで実施
